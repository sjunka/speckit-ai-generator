...
- [ ] T030 [US4] Set up the real accounts — the one task that touches them. Write `.env.local.example` from §4 verbatim (the appendix's `.gitignore` already carries `!.env.local.example`/`!.env*.example` so it commits cleanly — if your `.gitignore` doesn't have those two lines, T002 didn't copy the appendix verbatim; fix that first), then set up and document in the README: **Clerk** (application, Google and email sign-in, keys, `publicMetadata.role = "admin"` on the owner and that user's id in `OWNER_ID`), **MongoDB Atlas** (free M0 cluster, database `ia-generator`, network access `0.0.0.0/0` because Vercel Hobby has no fixed egress IP — never commit the connection string), **Vercel Blob** (a store and its read-write token), **Higgsfield** (key and secret, model ids checked against the explore page). → verify by hand with `.env.local` filled in: `/dashboard` renders for the admin user and 404s for a second non-admin user; toggling the switch off makes the next generate call return `503`; toggling it back on restores it
  ...
  **Nine keys, not eight.** `.env.local.example` carries a ninth line the
  original appendix did not: `NEXT_PUBLIC_CLERK_SIGN_IN_URL=/sign-in`. Without
  it `auth.protect()` sends an anonymous visitor to Clerk's *hosted* domain
  (`<slug>.accounts.dev`) and the `app/sign-in` screen T012 builds is only ever
  reached from the landing's call to action — two different sign-in screens
  depending on how you arrive. Caught in the rehearsal against real keys, not
  by any test: the suite mocks Clerk, so no test can see it. T039's list of
  environment variables in the Vercel project settings grows by this one too.
...
- [ ] T038 Run the exactness check: copy `scripts/build-appendix.mjs` into the repository, run `npm run docs:appendix`, then `diff docs/REPLICATION-APPENDIX.md <the original appendix>`. Every difference is either a file that is wrong or a deliberate change — there is no third case, so resolve each one explicitly. An empty diff is the definition of done
  ...
  **`build-appendix.mjs` reads `git ls-files`, so a file has to be tracked to
  be seen.** Run this after the merge commit, not on an untracked working tree
  — otherwise it silently emits only the files git already knows about (the
  rehearsal got 65 instead of 80) and the diff is unreadable. `git add -A`
  without committing is enough to make the check meaningful.

  **The rehearsal also had to widen the script's `SKIP`.** This repository
  tracks `specs/` and `.specify/` and two PDF builders that the source
  repository did not, and none of them are the app; unfiltered they turned 80
  files into 106. That widening is in the appendix now. Expect the same fix in
  any repository that carries its spec-kit artefacts alongside the code.

  **Four deltas are deliberate and expected to survive the diff**: the ninth
  Clerk variable in `.env.local.example` (see T030), `.react-doctor-baseline`
  at `100` rather than `55` (T036 raises it and forbids lowering it),
  `package.json`'s name and dependency ranges (this repository is `ia-generator`
  per T001 and sits on newer patch releases), and `test/mongo-fake.js` carrying
  the `$gte` support T029 mandates — the original appendix predates that
  extension, so on this one file the appendix is the stale side and T029 wins.
