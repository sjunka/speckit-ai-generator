...
- **The assets come wrapped in objects, not as plain strings**: `images` is `[{ url }]` and `video` is `{ url }` — `status.images[0].url`, `status.video.url` (trap 21). Verified against the live API.
