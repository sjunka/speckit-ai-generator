...
- [X] T031 [US1] [US2] [US3] [US4] After the last block merges, run `npm run test:e2e` against `develop` for `e2e/landing.spec.js`, `e2e/wall.spec.js` and `e2e/gallery-requires-session.spec.js`. All three specs — 001's landing spec included, unchanged — must be green on the merge commit. A failure here is a merge defect, not an e2e defect: fix it where it belongs, not in `e2e/`
